...
function FavoriteTickers() {
  ...
  async function getFavorites() {
    try {
      ...
      const response = await axios.post(
        "http://localhost:3030/chatterbot/getFavorites",
        data
      );
  ...
  const removeTicker = async (id) => {
    ...
    const response = await axios.post(
      "http://localhost:3030/chatterbot/removeFavorite",
      data
    );
  ...
  if (data && data.length != 0) {
    return (
      <>
        {showMessageValid && (
          <Message valid={true} message={"Ativo removido dos favoritos"} />
        )}
        {showMessageInvalid && (
          <Message valid={false} message={"Erro removido dos favoritos"} />
        )}
              ...
              <div className="active" key={action.code}>
                ...
                <div className="btn">
                  <button
                    ...
                    onClick={() => removeTicker(action._id)}
                  >
                    <img
