  ...
  async function getPj() {
    try {
      const response = await axios.get(
        "http://localhost:3030/chatterbot/getBestPj"
      );
      ...
      const actualPricesResponse = await axios.post(
        "http://localhost:3030/chatterbot/tickerPriceArray",
        search
      );
      const actualPricesData = actualPricesResponse.data.prices;
      setActualPrices(actualPricesData);

      const justPricesResponse = await axios.post(
        "http://localhost:3030/chatterbot/tickerInfoJustPrice",
        search
      );
          ...
          <div className="active pj" key={action}>
            <div className="name">
              <span className="name-ticker">
                <strong>{action}</strong> | R${actualPrices[index]} | R$
                {justPrices[index]}
              </span>
            </div>
          </div>
